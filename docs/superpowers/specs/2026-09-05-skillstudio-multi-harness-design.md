...
日期：2026-09-05。状态：产品方向已批准，等待书面规格复核。决策：采用“独立验证工作台”方案。
...
- Codex CLI：兼容诊断、真实运行、事件采集和结果评价。
- Hermes Agent：兼容诊断、真实运行、事件采集和结果评价。
- DeepSeek Harness：静态兼容诊断、能力模型和适配器契约；真实运行进入后续版本。
...
3. 作者选定同一 Skill 版本，在 Codex 与 Hermes 中串行运行。
4. 系统保留原始事件并生成统一结果，用户完成评价和对比。
...
完成统一适配器、Codex/Hermes 探测与兼容诊断、DeepSeek 静态规则、能力矩阵和真实认证夹具。
...
- 首版 Codex 与 Hermes 真实运行，DeepSeek Harness 静态诊断和接口预留。
